...
export class example extends plugin {
  ...
  */
  async tlp (e) {
   let data_redis = await redis.get(`Yunzai:setlinshimsg:${e.user_id}_tlp`); //先获取这个逼 看看他今天有没有抽牌
	var new_sum = 1;
	if (data_redis) {
		if (JSON.parse(data_redis)[0].num >= sum) {
			e.reply(`你今天已经抽过${sum}份塔罗牌了请明天再来~`)
			return;
		}
		new_sum += JSON.parse(data_redis)[0].num; //次数累加
	}
//	var file = _path + '/resources/tlp/';
	let tlp = `
    {
      "0": {
  ...
  e.reply(msg);
	...
	redis.set(`Yunzai:setlinshimsg:${e.user_id}_tlp`, JSON.stringify(redis_data), { //把次数写入缓存防止一直抽卡
